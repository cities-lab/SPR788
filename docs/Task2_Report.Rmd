...
pacman::p_load("gridExtra", "knitr", "moments", "pastecs", "tidyverse",
               "printr", "scales", "magrittr", "stargazer", "pander",
               "splines", modelr)
```
...
res.gs_pwdvmt <- mm.df %>%
  add_formulas(fmlas) %>% 
  mutate(#train = map(train, ~resample_filter(.x, DVMT<quantile(DVMT, .99, na.rm=T))),
         ...
         #model = map(model, strip_model),
         rmse = map2_dbl(preds, map(test, resample_get, col_name="DVMT"), calc_rmse),
...
3. Combined model
```{r}
test <- mm.df %>% transmute(metro=metro, 
                            HOUSEID=map(test, resample_get, col_name="HOUSEID"),
                            DVMT=map(test, resample_get, col_name="DVMT"))
test_preds <- test %>% 
  left_join(res.gs_0dvmt %>% dplyr::select(metro, preds),  by="metro") %>% 
  left_join(res.gs_pwdvmt %>% dplyr::select(metro, preds), by="metro", suffix=c(".1", ".2")) %>% 
  unnest()

test_preds %>% group_by(metro) %>% 
  mutate(ZeroDVMT = 1 - rbinom(n(), 1, preds.1),
         DVMT.pred = preds.2 * ZeroDVMT) %>% 
  summarize(rmse=rmse(DVMT.pred, DVMT))
```
...
fmlas <- list(metro=~lm(I(AADVMT^0.38) ~ HHSIZE+WRKCOUNT+LogIncome+Age65Plus+ns(log1p(VehPerDriver), 3) +
              ...
              non_metro=~lm(I(AADVMT^0.38) ~ 
                              ...
                              log1p(TRPOPDEN)+log1p(EMPTOT_5)+D1B+D2A_EPHHM+D1B:D2A_EPHHM+D5ar, data= .))
...
fmlas <- list(    metro=~pscl::hurdle(AADVMT.int ~ HHSIZE+WRKCOUNT+CENSUS_R+LogIncome+Age65Plus + ns(log1p(VehPerDriver), 3) + 
                                ...
                                D3bpo4+D5cr+log1p(D5ar) | HHSIZE+WRKCOUNT+LogIncome+Age65Plus + ns(log1p(VehPerDriver), 3) + 
              ...
              non_metro=~pscl::hurdle(AADVMT.int ~ HHSIZE+WRKCOUNT+CENSUS_R+LogIncome+Age65Plus+ns(log1p(VehPerDriver), 3)+ 
                              log1p(TRPOPDEN)+log1p(EMPTOT_5)+D1D+D1B:D2A_EPHHM+log1p(D5ar) | 
                                WRKCOUNT+CENSUS_R+LogIncome+Age65Plus+ns(log1p(VehPerDriver), 3) + 
                              log1p(EMPTOT_5)+D1B:D2A_EPHHM+D5cr, 
                                  data=., na.action=na.omit, dist = "negbin"))
...
fmlas <- list(metro    =~lm(I((td.miles.Transit)^0.24) ~ log1p(AADVMT) + log1p(VehPerDriver)+HHSIZE+LIF_CYC+Age0to14+D1D+D2A_EPHHM + D3bmm4 + D3bpo4 +
                                  log1p(D5ar) + WRKCOUNT + Fwylnmicap + Tranmilescap + Tranmilescap:D4c + LogIncome + D3apo + D4c,
                             data=., na.action=na.omit),
